...
const GRAPH_COLORS = {
	nodeIndividual: 'var(--c-individual)',
	nodeFirm: 'var(--c-firm)',
	nodeEntity: 'var(--c-entity)',
	nodeStub: '#60a5fa',
	nodeDefault: '#475569',
	nodeBorder: '#fff',
	nodeLabel: '#1e293b',
	...
	nodePulse: '#ff9f1c',
	lineEmployedBy: '#2563eb',
	lineControls: '#ff0c0c',
	lineControlsHighlight: '#ff2222',
	lineDisclosure: '#f97316',
	lineNeutral: '#5e6268',
	linePreviousEmployment: '#5e6268',
};
...
const NON_GRAY_HOP_ANIMATION_MS = 720;
...
function saveSession() {
	...
	try {
		const hasRenderedNodes = Array.isArray(layoutNodes) && layoutNodes.length > 0;
		const hasRenderedLinks = Array.isArray(layoutLinks) && layoutLinks.length > 0;
...
function getCurrentRevealHops() {
	const hopsEl = document.getElementById('fg-reveal-hops') as HTMLSelectElement | HTMLInputElement | null;
	if (!hopsEl) return 1;
	const value = hopsEl.value;
	return value === 'all' ? 'all' : normalizeHighlightHops(value);
}

function incrementRevealHops(hops) {
	const normalizedHops = normalizeHighlightHops(hops);
	if (normalizedHops === 'all') return 'all';
	return normalizedHops + 1;
}

function getStoredHighlightHops(id) {
	return highlightedSelections.find((entry) => entry.id === id)?.hops ?? 1;
}

function getNextRevealHopsForNodeClick(id) {
	const existingEntry = highlightedSelections.find((entry) => entry.id === id);
	return existingEntry ? incrementRevealHops(existingEntry.hops) : 1;
}
...
function restoreHighlightStateFromSession(session, { delayMs = 0 }: { delayMs?: number } = {}) {
	...
	const restoreSelection = () => {
		...
		highlightedSelections = restoredHighlights
			.map((entry) => ({
				id: entry?.id,
				hops: normalizeHighlightHops(entry?.hops),
			}))
		...
		const runRemoteFetch = async () => {
			...
			try {
				...
				function addIndividualFromSource(src) {
					...
					const crd = String(parsed?.basicInformation?.individualId || parsed?.ind_source_id || parsed?.ind_crd || parsed?.person?.crd || '').trim();
					if (!crd) return;
					...
					if (existingGraphNode) {
						...
						const iaDisclosureFlag = parsed?.iaDisclosureFlag ?? parsed?.basicInformation?.iaDisclosureFlag;
						batchAllNodes.push(
			...
			hits.slice(0, MAX_HITS).map(async (hit) => {
				...
				if (crd && /^\d+$/.test(crd)) {
					try {
						...
						const personLabel = normalizePersonLabel(detail?.basicInformation?.name || src?.name || `CRD ${crd}`);
						...
						for (const e of [
							...
							const fid = String(e?.firmId || e?.firm_id || e?.firmIdNumber || '').trim();
							if (!fid) continue;
...
function updateSubsetInfo(shown, total) {
	const info = document.getElementById('fg-subset-info');
	const sel = document.getElementById('fg-subset-select') as HTMLSelectElement | null;
	if (info) info.textContent = `Showing ${shown.toLocaleString()} of ${total.toLocaleString()} nodes`;
...
// Cache stats fetched once and reused for the bottom status bar.
let _cacheStats = null;
function fetchCacheStats() {
	fetch('/api/finra/cache-stats')
		.then((r) => r.json())
		.then((data) => {
			if (data?.counts) {
				_cacheStats = data.counts; // { external (firms), national (seeds) }
				updateMeta();
			}
		})
		.catch(() => {});
...
function updateMeta(meta: { totalIndividuals?: number; totalFirms?: number; totalLinks?: number } = {}) {
	...
	if (bottomEl) {
		let cacheSeeds = _cacheStats?.national ?? '–';
		let cacheFirms = _cacheStats?.external ?? '–';
		...
		bottomEl.textContent =
			`Displayed: ${fmt(dispSeeds)} Seeds  ${fmt(dispFirms)} Firms  ${fmt(dispLinks)} Links` +
			`  / Redis cache: ${fmt(cacheSeeds)} Seeds ${fmt(cacheFirms)} Firms ${fmt(cacheLinks)} Links`;
	}
}
...
function getNodeCollisionRadius(node, nodeCount = layoutNodes?.length || 0) {
	const padding =
		...
		: nodeCount > 120 ? 18
		: 20;
	return (node?._vizHalf != null ? node._vizHalf : NODE_R[node?.group] || 10) + padding;
}
	...
	selection.each(function (d) {
		...
		['halo', 'fill'].forEach((pass) => {
			g.append('text')
				...
				.text(truncate(formatNodeLabel(d.label), 22));
		});
...
function isNonGrayExpansionLink(link) {
	...
	return link.relationship === 'employed_by' && isCurrentRegistration(link);
}

function buildLinkAdjacency(links, linkFilter = null) {
...
async function fetchExpansionDataForNodeIds(nodeIds: string[] = []) {
	const uniqueIds = Array.from(new Set<string>(nodeIds.filter(Boolean)));
	if (!uniqueIds.length) return;

	const results = await Promise.allSettled(
		uniqueIds.map(async (nodeId) => {
			const response = await fetch(`${BASE}/api/finra/expand/${encodeURIComponent(nodeId)}`);
	...
	if (mergedNodes.length || mergedLinks.length) {
		mergeIntoGraphData(mergedNodes, mergedLinks);
	}
}

async function ensureExpansionDataForHops(clickedNodeId, hops: number | 'all' = 1) {
	const depthLimit = hops === 'all' ? 100 : Number(normalizeHighlightHops(hops));
	if (!clickedNodeId || depthLimit < 1) return;

	const visitedIds = new Set([clickedNodeId]);
	let frontierIds = [clickedNodeId];
	let depth = 0;

	while (frontierIds.length && depth < depthLimit) {
		await fetchExpansionDataForNodeIds(frontierIds);
		depth += 1;
		if (depth >= depthLimit) break;

		const adjacency = buildLinkAdjacency(graphData?.links || []);
		...
		frontierIds.forEach((frontierId) => {
			(adjacency.get(frontierId) || []).forEach(({ nodeId }) => {
				...
				nextFrontierIds.push(nodeId);
			});
		});

		frontierIds = Array.from(new Set(nextFrontierIds));
	}
}

async function expandNodeThroughNonGrayHops(clickedNode) {
	...
	while (frontierIds.length && waveCount < 100) {
		waveCount += 1;
		await fetchExpansionDataForNodeIds(frontierIds);
...
async function handleNodeOpen(event, d) {
	...
	anchorNode(d);
	const nextHops = getNextRevealHopsForNodeClick(d.id);
	selectNode(d, { skipAutoExpand: true, highlightHops: nextHops });
	try {
		await expandFromServer(d, nextHops);
	} catch (err) {
		console.error('Progressive hop expansion failed:', err);
	} finally {
		reapplySelectionState();
		try {
			saveSession();
...
function selectNode(
	...
	options: {
		...
		highlightHops?: number | 'all';
		focus?: boolean;
		pulse?: boolean;
		focusDuration?: number;
	} = {},
) {
	const { persist = true, skipProfileSync = false, skipAutoExpand = false, highlightHops = null, focus = false, pulse = false, focusDuration = 600 } = options;
	if (selectionRestoreTimer) {
		clearTimeout(selectionRestoreTimer);
		selectionRestoreTimer = null;
	}
	...
	const hops =
		highlightHops != null ? normalizeHighlightHops(highlightHops)
		: skipAutoExpand ? getStoredHighlightHops(d.id)
		: getCurrentRevealHops();
	...
	if (!skipAutoExpand) {
		...
		expandFromServer(d, hops).finally(() => {
			reapplySelectionState();
			try {
				saveSession();
			} catch (e) {
...
function placeNodesNearConnections(anchorNode, nodesToPlace, candidateLinks, hopDistances) {
	...
	return orderedNodes
		.map((node) => {
			...
			const preferredDistance = 45 + (hopDistance - 1) * 20 + sharedAnchorCount * 10;
			const maxDistanceFromAnchor = Math.min(120, getAnchorDistanceLimit(hopDistance));
...
async function expandFromServer(clickedNode, hops: number | 'all' = 1) {
	try {
		await ensureExpansionDataForHops(clickedNode?.id, hops);
	} catch {
...
function revealNeighbors(
	...
	const newNodes = placeNodesNearConnections(
		clickedNode,
...
function updateShortDetail(d) {
	...
	const label = d?.label || d?.name || 'Selected node';
	const suffix = id ? ` • ${id}` : '';
	...
	linkSel.each(function (d) {
		...
		if (connected) {
			sel
				...
				.attr('stroke-opacity', connectedToRoot ? 1 : 0.92)
				.attr(
					'stroke-width',
					...
					: isCurrentRegistration(d) ?
						connectedToRoot ? 1.7
						:	1.35
					: connectedToRoot ? 1.25
					: 1,
				);
		} else {
			sel.style('opacity', 0.22).style('stroke-opacity', null).attr('stroke', getLinkColor(d)).attr('stroke-opacity', 0.18).attr('stroke-width', 0.5);
		}
	});
}
...
function renderFirmDetail(d) {
	...
	const bcRawUrl = firmId ? `https://api.brokercheck.finra.org/search/firm/${encodeURIComponent(firmId)}?hl=true&nrows=12&query=&start=0&wt=json` : null;
