...
function allowedSecretMatches(req: NextRequest) {
	const expected = process.env.ADMIN_PUSH_SECRET || process.env.PUSH_SECRET || '';
	if (!expected) return false;
	const header = req.headers.get('x-admin-secret') || req.headers.get('authorization') || '';
...
export async function POST(req: NextRequest) {
	try {
		...
		if (typeof body?.firmId === 'string' && body.firmId.trim()) firmIds.push(String(body.firmId).trim());
		if (Array.isArray(body?.firmIds)) for (const f of body.firmIds) if (f) firmIds.push(String(f).trim());
		if (!firmIds.length) return NextResponse.json({ ok: false, error: 'no firmId(s) provided' }, { status: 400 });
		...
		for (const firmId of firmIds) {
			try {
				...
				// remove empty sentinel if present
				try {
					const redis = getRedisClient();
					if (redis) {
...
export async function GET(req: NextRequest) {
	...
	try {
		if (!allowedSecretMatches(req)) return NextResponse.json({ ok: false, error: 'unauthorized' }, { status: 401 });
		const url = new URL(req.url);
		const firmId = url.searchParams.get('firmId') || undefined;
		if (!firmId) return NextResponse.json({ ok: false, error: 'missing firmId query' }, { status: 400 });
		// proxy to POST behavior for single firm
		return await POST(new Request(req.url, { method: 'POST', headers: req.headers, body: JSON.stringify({ firmId }) }) as unknown as NextRequest);
	} catch (err: any) {
		return NextResponse.json({ ok: false, error: String(err?.message || err) }, { status: 500 });
	}
}
