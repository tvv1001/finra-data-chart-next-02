...
export async function GET(request: NextRequest, { params }: { params: Promise<{ crd: string }> }) {
	...
	try {
		...
		// Previously SEC was passed as primary to mergePreferPrimary which
		// caused SEC fields to win over FINRA when both existed.
		const detail: any =
			finraDetail ?
				secDetail ? mergePreferPrimary(finraDetail, secDetail)
				:	finraDetail
			:	secDetail;
