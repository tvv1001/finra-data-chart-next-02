...
import {
  buildJsonDisplayTree,
  coerceStructuredValue,
  normalizeRenderablePayload,
  renderJsonForDisplay,
} from "../../lib/dashboard-json";
...
export function parseDashboardSelectionFromUrl(
  urlString: string,
): UrlSelectionInput | null {
  try {
    ...
    const pathMatch = /^\/dashboard\/(individual|firm)\/([^/?#]+)\/?$/i.exec(
      url.pathname,
    );
    if (pathMatch) {
      const entity: "individual" | "firm" =
        pathMatch[1].toLowerCase() === "firm" ? "firm" : "individual";
...
function extractNumericCrdsFromText(text: string): string[] {
  ...
  const directMatches = Array.from(
    raw.matchAll(
      /(?:^|[\s:;#-])(?:crd|crd\s*#|crd\s*id|individual\s*crd|firm\s*crd|individual\s*id|firm\s*id|id)\s*[:#-]?\s*(\d{1,10})/gi,
    ),
  );
  ...
  const fallbackMatches = Array.from(
    raw.matchAll(
      /\b(?:crd|crd\s*#|crd\s*id|individual\s*crd|firm\s*crd|individual\s*id|firm\s*id)\b[^0-9]{0,10}(\d{1,10})\b/gi,
    ),
  );
...
function buildGraphHrefForEntity(
  entity: "individual" | "firm" | null | undefined,
  ...
  if (entity === "individual")
    return `/individual/${encodeURIComponent(normalizedId)}`;
  return null;
}
...
function getLatestGraphHrefFromHistory(entries: LocalHistoryEntry[]) {
  ...
  const sorted = entries
    ...
    .sort(
      (left, right) =>
        new Date(right.lastVisitedAt || right.fetchedAt).getTime() -
        new Date(left.lastVisitedAt || left.fetchedAt).getTime(),
    );
...
function collectSelectedNodeIdsForGraphHref(
  localHistory: LocalHistoryEntry[],
  graphClickHistory: SelectionLogEntry[],
) {
  ...
  for (const entry of Array.isArray(graphClickHistory)
    ? graphClickHistory
    : []) {
...
function parseQueueQueries(input: string) {
  ...
  const PREFIX_NUMERIC_REGEX =
    /^(?:crd|crd\s*#|crd\s*id|individual\s*crd|firm\s*crd|individual\s*crd\s*:|firm\s*crd\s*:)\s*(\d{1,10})$/i;
...
export function createQueueTerminalLogId(
  kind: string,
  index: number,
  step: number,
) {
...
export function computeQueryFetchCounts(
  resolution: Array<{ query?: string; crds?: string[] }>,
  fetchedItems: Array<{ crd?: string; status?: string }>,
) {
  ...
  for (const entry of resolution) {
    ...
    const crdSet = new Set(
      (Array.isArray(entry?.crds) ? entry.crds : [])
        .map((value) => String(value || "").trim())
        .filter(Boolean),
    );

    const addedCount = fetchedItems.filter(
      (item) =>
        crdSet.has(String(item?.crd || "").trim()) &&
...
export function computeQuerySaveStats(
  ...
  for (const entry of resolution) {
    ...
    const matchingItems = fetchedItems.filter((item) =>
      crdSet.has(String(item?.crd || "").trim()),
    );
    const savedRecordKeys = new Set(
      matchingItems
        ...
        .map((item) =>
          String(
            item?.cardKey ||
              `${String(item?.type || "individual")}:${String(item?.crd || "").trim()}`,
          ),
        ),
    );
    const updatedExistingRecordKeys = new Set(
      matchingItems
        .filter(
          (item) =>
            item?.newSourceSaved === true && item?.newRecordSaved !== true,
        )
        .map((item) =>
          String(
            item?.cardKey ||
    ...
    stats.set(query, {
      fetchedCount: matchingItems.filter(
        (item) => String(item?.status || "") === "ok",
      ).length,
      skippedCount: matchingItems.filter(
        (item) => String(item?.status || "") === "skipped",
      ).length,
      savedSourceCount: matchingItems.filter(
        (item) => item?.newSourceSaved === true,
      ).length,
      savedRecordCount: savedRecordKeys.size,
      updatedExistingSourceCount: matchingItems.filter(
        (item) =>
          item?.newSourceSaved === true && item?.newRecordSaved !== true,
      ).length,
      updatedExistingRecordCount: updatedExistingRecordKeys.size,
      errorCount: matchingItems.filter(
        (item) => String(item?.status || "") === "error",
      ).length,
    });
  }
...
export function describeQuerySaveChange(stats?: {
  savedRecordCount?: number;
  updatedExistingRecordCount?: number;
  updatedExistingSourceCount?: number;
}) {
  const newRecordCount = Number(stats?.savedRecordCount || 0);
  const updatedExistingRecordCount = Number(
    stats?.updatedExistingRecordCount || 0,
  );
  const updatedExistingSourceCount = Number(
    stats?.updatedExistingSourceCount || 0,
  );
...
export function shouldShowQueueCardError(card: {
  ...
  return (
    ...
    card.sources.some((entry) => String(entry?.status || "") === "error")
  );
}
...
export function shouldShowQueueCardSkipped(card: {
  ...
  return (
    Array.isArray(card.sources) &&
    !shouldShowQueueCardError(card) &&
    card.sources.some((entry) => String(entry?.status || "") === "skipped")
  );
}

export function describeDashboardRequestFailure(options: {
  status?: number;
  ...
  if (
    combined.includes("aborted without reason") ||
...
function extractEntityDetailFromPayload(
  payload: any,
  entity: "individual" | "firm",
  crd: string,
) {
  ...
  if (entity === "individual") {
    const firstName =
      normalized.bc?.firstName ||
      normalized.ia?.firstName ||
      normalized.basicInformation?.firstName ||
      "";
    const lastName =
      normalized.bc?.lastName ||
      normalized.ia?.lastName ||
      normalized.basicInformation?.lastName ||
      "";
    const name =
      ...
      "";
    const bcScope =
      normalized.bc?.bcScope || normalized.basicInformation?.bcScope || "N/A";
    const iaScope =
      normalized.ia?.iaScope || normalized.basicInformation?.iaScope || "N/A";
    ...
    return {
      ...
      status:
        [bcScope, iaScope].filter((s) => s && s !== "N/A").join(" • ") ||
        "No status",
    };
  }
  ...
  if (entity === "firm") {
    ...
    const doingBusinessAs =
      normalized.doingBusinessAs ||
      normalized.basicInformation?.doingBusinessAs ||
      normalized.dba ||
      normalized.basicInformation?.dba ||
      "";
...
function inferEntityTypeFromNewCrd(item: {
  type?: string;
  scopes?: string[];
}): "individual" | "firm" {
  const typeText = String(item?.type || "")
    .trim()
    .toLowerCase();
  const scopesText = Array.isArray(item?.scopes)
    ? item.scopes.join(" ").toLowerCase()
    : "";
...
function buildReadableSummaryRows(payload: Record<string, any>) {
  ...
  push(
    ...
    payload.name ||
      payload.fullName ||
      payload.legalName ||
      payload.basicInformation?.legalName,
  );
  push(
    "Status",
    payload.status ||
      ...
      payload.bcScope ||
      payload.iaScope ||
      payload.basicInformation?.bcScope ||
      payload.basicInformation?.iaScope,
  );
  push(
    "City",
    payload.city ||
      ...
      payload.businessAddress?.city,
  );
  push(
    "State",
    payload.state ||
      payload.mailingAddress?.state ||
      payload.businessAddress?.state,
  );
  push(
    "Country",
    payload.country ||
      payload.mailingAddress?.country ||
      payload.businessAddress?.country,
  );
  push(
    "SEC",
    payload.iaSecNumber ||
      payload.secNumber ||
      payload.basicInformation?.iaSecNumber,
  );
...
function unwrapRecordPayload(input: unknown): any {
  ...
  if (
    payload.finraBrokerCheck &&
    typeof payload.finraBrokerCheck === "object"
  ) {
    return unwrapRecordPayload(payload.finraBrokerCheck);
  }
  if (
    payload.secInvestmentAdvisor &&
    typeof payload.secInvestmentAdvisor === "object"
  ) {
  ...
  const firstHit = Array.isArray((payload.hits as any)?.hits)
    ? (payload.hits as any).hits[0]
    : null;
  if (firstHit && typeof firstHit === "object") {
    const source = (firstHit as any)._source;
    if (source && typeof source === "object") {
      if ((source as any).content != null)
        return unwrapRecordPayload((source as any).content);
      if ((source as any).iacontent != null)
        return unwrapRecordPayload((source as any).iacontent);
      return unwrapRecordPayload(source);
    }
  }

  if (
    (payload as any)._source &&
    typeof (payload as any)._source === "object"
  ) {
...
function looksLikeGenericEntityLabel(value: unknown) {
  ...
  if (!text) return true;
  return (
    /^(firm|individual)\s+\d{1,10}$/i.test(text) ||
    /^employment\s+\d+$/i.test(text) ||
    /^owner\s+\d+$/i.test(text) ||
    /^result$/i.test(text)
  );
}
...
function resolveEntityNodeLabel(
  ...
  const row = record || {};
  const basic =
    row?.basicInformation && typeof row.basicInformation === "object"
  ...
  if (typeof indexFallback === "number")
    return `${entity === "firm" ? "Firm" : "Individual"} ${indexFallback + 1}`;
...
function extractCurrentBranchOfficeAddress(body: Record<string, any>): string {
  const employments = [
    ...toArray(body?.currentEmployments),
    ...toArray(body?.currentIAEmployments),
    ...toArray(body?.currentEmployment),
  ];
  for (const employment of employments) {
    const branches = toArray(employment?.branchOfficeLocations);
    const located =
      branches.find((b) => toText(b?.locatedAtFlag).toUpperCase() === "Y") ||
      branches[0];
    if (located) {
...
function extractDisplayNameFromNewCrd(
  entry: NewCrdEntry,
  entity: "individual" | "firm",
) {
  const combinedPersonName = [toText(entry.firstName), toText(entry.lastName)]
    .filter(Boolean)
    .join(" ")
    .trim();
...
function getNewCrdSources(
  entry: NewCrdEntry,
  entity: "individual" | "firm",
  localHistory?: LocalHistoryEntry[],
): { hasFinra: boolean; hasSec: boolean } {
  const scopes = (entry.scopes || []).map((s) => String(s).toLowerCase());
  const typeStr = String(entry.type || "").toLowerCase();
  let hasFinra =
    scopes.includes("finra") ||
    scopes.includes("bc") ||
    typeStr.includes("finra");
  let hasSec =
    scopes.includes("sec") || scopes.includes("ia") || typeStr.includes("sec");

  if (localHistory) {
    const hist = localHistory.find(
      (h) => h.id === entry.id && h.entity === entity,
    );
    if (hist && Array.isArray(hist.sources)) {
      const histSources = hist.sources.map((s) =>
        String(s.source).toLowerCase(),
      );
...
function formatMainPanelTitle(options: {
  source: SearchResultSource;
  entity: "individual" | "firm";
  id: string;
  name?: string | null;
  ...
  const titleName =
    toText(options.name) ||
    getRecordDisplayName(options.payload, options.entity, options.id);
...
function extractJurisdictionCards(body: Record<string, any>) {
  const normalizedBody = coerceStructuredValue(body) as
    Record<string, any> | undefined;
  const source = Array.isArray(normalizedBody)
    ? normalizedBody
    : findNestedValueByKey(normalizedBody ?? body, [
        "jurisdictions",
        "stateNoticeDetails",
        "stateNoticeRecords",
        "stateNoticeHistory",
  ...
  return entries
    ...
    .map((entry) => {
      const record = coerceStructuredValue(entry) as
        Record<string, any> | undefined;
      ...
      const meta = pickFirstNonEmpty(
        record?.status,
        record?.noticeStatus,
        record?.noticeStatusText,
      );
...
function extractBrochureCards(body: Record<string, any>) {
  const normalizedBody = coerceStructuredValue(body) as
    Record<string, any> | undefined;
  const brochureContainer = Array.isArray(normalizedBody)
    ? normalizedBody
    : findNestedValueByKey(normalizedBody ?? body, [
        "brochures",
        "brochuredetails",
  ...
  const entries = Array.isArray(brochureContainer)
    ? brochureContainer
    : brochureContainer &&
        typeof brochureContainer === "object" &&
        Array.isArray(
  ...
  return entries
    ...
    .map((entry) => {
      ...
      const title = pickFirstNonEmpty(
        record?.brochureName,
        record?.name,
        record?.brochureTitle,
      );
      const meta = pickFirstNonEmpty(
        record?.brochureVersionID,
        record?.versionId,
        record?.version,
      );
      const submitted = pickFirstNonEmpty(
        record?.dateSubmitted,
        record?.submittedDate,
        record?.date,
      );
      const confirmed = pickFirstNonEmpty(
      ...
      return title
        ? { title, meta: meta ? `ID #${meta}` : "", subtitle }
        : null;
    })
    .filter(Boolean) as Array<{
    title: string;
...
export function extractConnectionCards(
  body: Record<string, any>,
  key: "currentConnections" | "previousConnections",
) {
  const normalizedBody = coerceStructuredValue(body) as
    Record<string, any> | undefined;
  const candidateKeys =
  ...
  let source = Array.isArray(normalizedBody)
    ? normalizedBody
    : findNestedValueByKey(normalizedBody ?? body, candidateKeys);

  if (!source) {
    const rawConns = findNestedValueByKey(normalizedBody ?? body, [
      "connections",
      "firmConnections",
      "affiliatedFirms",
    ...
    if (Array.isArray(rawConns)) {
      const isInactive = (c: any) =>
        /previous|former|terminated|inactive/i.test(
          String(
            c.relationship ||
              c.status ||
              c.firmStatus ||
  ...
  return entries
    ...
    .map((entry) => {
      ...
      if (!record || typeof record !== "object" || Array.isArray(record))
        return null;
      const crd = pickFirstValidCrd(
        record?.crdNumber,
        record?.crd,
        record?.individualId,
        record?.personId,
        record?.firmId,
        record?.organizationCrd,
        record?.sourceId,
      );
      ...
      const startDate = pickFirstNonEmpty(
        ...
        record?.startDate,
        record?.fromDate,
        record?.registrationBeginDate,
      );
      const endDate = pickFirstNonEmpty(
        record?.endDate,
        record?.toDate,
      ...
      const dateText =
        startDate && endDate
          ? `${startDate} - ${endDate}`
          : pickFirstNonEmpty(startDate, endDate);
      const addressText = pickFirstNonEmpty(
        ...
        [toText(record?.city), toText(record?.state)]
          .filter(Boolean)
          .join(", "),
      );
      const subtitle = [dateText, addressText].filter(Boolean).join(" • ");
      const otherNamesArr = Array.isArray(record?.otherNames)
        ? record.otherNames
...
export function extractNoticeFilingsCards(body: Record<string, any>) {
  ...
  return entries
    ...
    .map((entry) => {
      ...
      const detail = pickFirstNonEmpty(
        record?.description,
        record?.details,
        record?.summary,
      );
...
function extractSearchResultAddress(item: SearchResult): string {
  ...
  const employments = [
    ...(Array.isArray(item?.ind_current_employments)
      ? item.ind_current_employments
      : []),
    ...(Array.isArray(item?.ind_ia_current_employments)
      ? item.ind_ia_current_employments
      : []),
    ...(Array.isArray(item?.currentEmployments) ? item.currentEmployments : []),
    ...(Array.isArray(item?.currentIAEmployments)
      ? item.currentIAEmployments
      : []),
    ...
    ...(Array.isArray(item?.previousEmployments)
      ? item.previousEmployments
      : []),
    ...(Array.isArray(item?.previousIAEmployments)
      ? item.previousIAEmployments
      : []),
  ];
...
function extractSearchResultOtherNames(item: SearchResult): string[] {
  const raw =
    item?.otherNames ||
    item?.basicInformation?.otherNames ||
    item?.ind_other_names ||
    item?.firm_other_names ||
    item?.aliases;
...
async function hydrateSearchResultCard(
  card: SearchResultCard,
): Promise<SearchResultCard> {
  try {
    const url =
      card.entity === "individual"
        ? `/api/finra/individual/${encodeURIComponent(card.id)}?merged=1`
        : `/api/finra/firm/${encodeURIComponent(card.id)}?merged=1`;
    const res = await fetch(url, { headers: { Accept: "application/json" } });
    if (!res.ok) return card;
    const detail = await res.json().catch(() => null);
    if (!detail || detail?.found === false) return card;
    const merged =
      detail?.merged || detail?.finraNode || detail?.secNode || detail || {};
    ...
    const address =
      extractSearchResultAddress(merged) ||
      extractSearchResultAddress(basic) ||
      card.address;
    const otherNames = extractSearchResultOtherNames(basic).length
      ? extractSearchResultOtherNames(basic)
      : extractSearchResultOtherNames(merged).length
        ? extractSearchResultOtherNames(merged)
        : card.otherNames;
    const detailText =
      extractSearchResultDetail(merged) ||
      extractSearchResultDetail(basic) ||
      card.detail;
...
function OrphanProfileLinks({
  parentCrd,
  parentType = "firm",
}: {
  parentCrd: string;
  parentType?: "individual" | "firm";
}) {
  const [status, setStatus] = useState<{ finra: boolean; sec: boolean } | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    fetch(
      isParentIndividual
        ? `/api/finra/individual/${parentCrd}`
        : `/api/finra/firm/${parentCrd}`,
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  if (!status)
    return (
      <div style={{ fontSize: "13px", color: "#64748b" }}>
        Validating parent {isParentIndividual ? "individual" : "firm"}{" "}
        sources...
      </div>
    );

  if (!status.finra && !status.sec)
    return (
      <div style={{ fontSize: "13px", color: "#64748b" }}>
        No external parent links available.
      </div>
    );
  ...
  const promise = (async () => {
    try {
      const res = await fetch(
        `/api/finra/firm/${encodeURIComponent(crd)}?merged=1`,
        { headers: { Accept: "application/json" } },
      );
      if (!res.ok) return null;
      const data = await res.json().catch(() => null);
      const merged =
        data?.merged || data?.finraNode || data?.secNode || data || {};
      const names = extractSearchResultOtherNames(
        merged?.basicInformation || merged || {},
      );
...
function DashboardPageInner() {
  ...
  useEffect(() => {
    try {
      const allow =
        ...
        process.env.NEXT_PUBLIC_ALLOW_HEAL_QUERY === "1";
      if (!allow) return;
  ...
  const [externalRawDir, setExternalRawDir] = useState(
  ...
  const [currentRecordSource, setCurrentRecordSource] = useState<
    "finra" | "sec" | null
  >(null);
  const [currentRecordEntity, setCurrentRecordEntity] = useState<
    "individual" | "firm" | null
  >(null);
  ...
  const [terminalLogs, setTerminalLogs] = useState<
    { id: string; text: string; type: "info" | "error" | "warn" | "success" }[]
  >([]);
  ...
  const [activeCardSourceKey, setActiveCardSourceKey] = useState<string | null>(
    null,
  );
  ...
  const [graphClickHistory, setGraphClickHistory] = useState<
    SelectionLogEntry[]
  >([]);
  ...
  const [isSelectionHistoryEditMode, setIsSelectionHistoryEditMode] =
    useState(false);
  const [selectedHistoryIds, setSelectedHistoryIds] = useState<Set<string>>(
    new Set(),
  );
  const [isGraphClickHistoryEditMode, setIsGraphClickHistoryEditMode] =
    useState(false);
  const [selectedGraphClickIndexes, setSelectedGraphClickIndexes] = useState<
    Set<number>
  >(new Set());
  ...
  const [connectionsLoadingFirmId, setConnectionsLoadingFirmId] = useState<
    string | null
  >(null);
  ...
  const [connectionsFilterTags, setConnectionsFilterTagsState] = useState<
    string[]
  >([]);
  ...
  const parsedCrds = useMemo(
    () => queueQueries.filter((value) => /^\d{1,10}$/.test(value)),
    [queueQueries],
  );
  const queueQueryLines = useMemo(
    () => submittedQueueQueries,
    [submittedQueueQueries],
  );
  ...
  async function loadNewCrdsFromRedis() {
    try {
      ...
      if (data?.ok) {
        ...
        try {
          console.debug(
            "[dashboard] loadNewCrdsFromRedis: fetched",
            data.newCrds,
          );
  ...
  const handleConfirmSaveTemplate = () => {
    const name =
      newTemplateName.trim() || `Template ${savedTemplates.length + 1}`;
  ...
  useEffect(() => {
    ...
    const cacheKey = mainJson
      ? `main:${mainJsonLabel}`
      : `result:${String((result as any)?.ok)}:${String((result as any)?.error || "")}`;
    ...
    const compute = () => {
      ...
      try {
        ...
        if (!cancelled) {
          setCodeBlock(
            String(error?.message || error || "Failed to render JSON"),
          );
          setJsonTree(null);
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const isAlreadySelectedEntity =
      currentRecordId === routeSelection.id &&
      currentRecordEntity === routeSelection.entity;
    ...
    if (
      activeLoadKey &&
      activeLoadKey.startsWith(`${routeSelection.entity}:${routeSelection.id}:`)
    )
      return;
    ...
    const card: QueueCard = {
      ...
      sources: (routeSelection.availableSources || [routeSelection.source]).map(
        (source) => ({
          source,
          status: "unknown",
        }),
    ...
    routeSelectionEntityIdKey,
    routeSelection?.source,
    currentRecordId,
    currentRecordEntity,
    popNonce,
  ]);
  ...
  const queueMetaText = useMemo(() => {
    ...
    const filterSuffix =
      queueCrdFilter.trim().length > 0
        ? ` Filtered: ${filteredTotal.toLocaleString()} matched.${filteredTotal === 0 ? " (No cached match yet)" : ""}`
        : "";
  ...
  const persistenceNotice = useMemo(() => {
    if (queueMetaStats.persistenceNotice)
      return queueMetaStats.persistenceNotice;
  ...
  const uniqueCrdCounts = useMemo(() => {
    if (queueMetaStats.inventoryTotals) {
      const cached =
        (queueMetaStats.inventoryTotals as any).cachedCrdCount || 0;
      const cachedParsed =
        cached && cached !== "Not Found" && cached !== "Error"
          ? parseInt(cached, 10)
          : NaN;
      return {
        individuals: queueMetaStats.inventoryTotals.people,
        firms: queueMetaStats.inventoryTotals.firms,
        total: !isNaN(cachedParsed)
          ? cachedParsed
          : queueMetaStats.inventoryTotals.unique,
  ...
  const hasInventorySummary = useMemo(() => {
    return (
      queueMetaStats.totalCount > 0 ||
      queueMetaStats.totalCacheKeys > 0 ||
      localHistory.length > 0 ||
      uniqueCrdCounts.individuals > 0 ||
      uniqueCrdCounts.firms > 0
    );
  }, [
    localHistory.length,
    queueMetaStats.totalCacheKeys,
    queueMetaStats.totalCount,
    uniqueCrdCounts.firms,
    uniqueCrdCounts.individuals,
  ]);
  ...
  const detailedMainRecord = useMemo(() => {
    ...
    const basic =
      body.basicInformation && typeof body.basicInformation === "object"
    ...
    const sortEmployment = (arr: any[]) => {
      return arr.sort((a, b) => {
        const getSortDate = (row: any) => {
          const d = pickFirstNonEmpty(
            row.registrationBeginDate,
            row.effectiveDate,
            row.startDate,
          );
    ...
    const previousEmployment = sortEmployment([
      ...toArray(body.previousEmployments),
      ...toArray(body.previousIAEmployments),
    ]);
    const registrationCards = extractRegistrationCards(body);
    const currentConnectionCards = extractConnectionCards(
      body,
      "currentConnections",
    );
    const previousConnectionCards = extractConnectionCards(
      body,
      "previousConnections",
    );

    const stateExams = toArray(body.stateExamCategory).concat(
      toArray(body.stateExams),
    );
    const productExams = toArray(body.productExamCategory).concat(
      toArray(body.productExams),
    );
    const principalExams = toArray(body.principalExamCategory).concat(
      toArray(body.principalExams),
    );

    const registrations =
      body.registrations && typeof body.registrations === "object"
        ? body.registrations
        : {};
    ...
    const mainObj =
      typeof mainJson === "object" && mainJson !== null
        ? (mainJson as any)
        : {};
    const showFinra =
      mainObj.hasFinraData === true
        ? true
        : mainObj.hasFinraData === false
          ? false
          : currentRecordEntity === "individual"
            ? hasIndividualSourceCoverage(body, "finra")
            : hasFirmSourceCoverage(body, "finra");
    const showSec =
      mainObj.hasSecData === true
        ? true
        : mainObj.hasSecData === false
          ? false
          : currentRecordEntity === "individual"
            ? hasIndividualSourceCoverage(body, "sec")
    ...
    const crs =
      (body.crs || body.Crs || body.CRS) &&
      typeof (body.crs || body.Crs || body.CRS) === "object"
        ? body.crs || body.Crs || body.CRS
        : null;
    const affiliateDisclosures =
      (body.affiliateDisclosures || body["Affiliate Disclosures"]) &&
      typeof (body.affiliateDisclosures || body["Affiliate Disclosures"]) ===
        "object"
        ? body.affiliateDisclosures || body["Affiliate Disclosures"]
        : null;
    const bdDisclosureFlag = pickFirstNonEmpty(
      body.bdDisclosureFlag,
      body.bd_disclosure_flag,
      basic.bdDisclosureFlag,
    );
    const iaDisclosureFlag = pickFirstNonEmpty(
      body.iaDisclosureFlag,
      body.ia_disclosure_flag,
      basic.iaDisclosureFlag,
    );
    const brochuresPart2Exempt = pickFirstNonEmpty(
      body.brochures?.part2ExemptFlag,
      body.part2ExemptFlag,
    );

    const bcScope = pickFirstNonEmpty(
      basic.bcScope,
      body.bcScope,
      basic.brokerCheckScope,
      body.brokerCheckScope,
      body.bc_scope,
    );
    const iaScope = pickFirstNonEmpty(
      basic.iaScope,
      body.iaScope,
      basic.secScope,
      body.secScope,
    ...
    const finraActive = !showFinra
      ? ""
      : bcScope
        ? `FINRA: ${bcScope}`
        : "FINRA: Active";
    const secActive = !showSec
      ? ""
      : iaScope
        ? `SEC: ${iaScope}`
        : "SEC: Active";
    const subtitle =
      otherNames.length > 0 && currentRecordEntity === "individual"
        ? otherNames[0]
        : "";

    const directOwners = toArray(body.directOwners).concat(
      toArray(body.directOwnersExecutiveOfficers),
    );
    ...
    if (Array.isArray(body.disclosures)) {
      for (const d of body.disclosures) {
        const type = String(
          d?.disclosureType || d?.type || d?.disclosure || "",
        ).trim();
        if (!type) continue;
        const count = Number(d?.disclosureCount ?? d?.count ?? 0);
        const increment = count > 0 ? count : 1;
        groupedDisclosures.set(
          type,
          (groupedDisclosures.get(type) || 0) + increment,
        );
      }
    }
    ...
    const disclosureSummary = Array.from(groupedDisclosures.entries()).map(
    ...
    return {
      name:
        ...
        extractEntityDetailFromPayload(
          content,
          currentRecordEntity,
          currentRecordId,
        )?.name ||
  ...
  const additionalDetailsSourceLabel = useMemo(() => {
    if (!mainJson || typeof mainJson !== "object") {
      return currentRecordSource
        ? String(currentRecordSource).toUpperCase()
        : "";
    }
  ...
  const displayCards = useMemo<QueueCard[]>(() => {
    ...
    const filtered = tokens.length
      ? localHistory.filter((e) =>
          tokens.some((t) => e.id === t || e.id.includes(t)),
        )
      : localHistory;
    return filtered
      .slice()
  ...
  const filteredNewCrds = useMemo(() => {
    ...
    const tokens = token
      ...
      .map((value) => value.trim())
      .filter(Boolean);
    return newCrds.filter((item) =>
      tokens.some((value) => item.id === value || item.id.includes(value)),
    );
  ...
  function recordHistoryEntry({
    ...
    setLocalHistory((prev) => {
      ...
      const resolvedName =
        incomingName && !looksLikeGenericEntityLabel(incomingName)
          ? incomingName
          : existingName && !looksLikeGenericEntityLabel(existingName)
            ? existingName
      ...
      const combined = [updatedEntry, ...nextEntries].slice(
        0,
        LOCAL_HISTORY_MAX,
      );
      try {
  ...
  async function setMainViewFromSearch(card: SearchResultCard) {
    const orderedSources: SearchResultSource[] =
      card.source === "sec" ? ["sec", "finra"] : ["finra", "sec"];
  ...
  async function openHistoryEntry(entry: LocalHistoryEntry) {
    const primarySource =
      entry.sources.find((item) => item.source === "finra")?.source ||
  ...
  function deleteSelectedHistoryEntries() {
    ...
    setLocalHistory((prev) => {
      const next = prev.filter(
        (entry) => !selectedHistoryIds.has(`${entry.entity}:${entry.id}`),
      );
      if (typeof window !== "undefined") {
        try {
  ...
  async function openQueueCard(card: QueueCard) {
    const preferredSource =
      card.sources.find((entry) => entry.source === "finra")?.source ||
      card.sources[0]?.source ||
      "finra";
  ...
  async function openNewCrdEntry(entry: NewCrdEntry) {
    ...
    const historyMatch = localHistory.find(
      (item) => item.entity === entity && item.id === entry.id,
    );
    if (historyMatch) {
    ...
    if (scopeText.includes("finra") || scopeText.includes("bc"))
      sources.push({ source: "finra", status: "unknown" });
    if (scopeText.includes("sec") || scopeText.includes("ia"))
      sources.push({ source: "sec", status: "unknown" });
    ...
    await loadQueueSourceJson(
      {
        ...
        sources,
        name:
          historyNameMap.get(`${entity}:${entry.id}`) ||
          extractDisplayNameFromNewCrd(entry, entity),
      },
      sources[0].source,
    );
  }
  ...
  function isSelectedCardSource(card: QueueCard, source: SearchResultSource) {
    return (
      currentRecordId === card.id &&
      currentRecordEntity === card.entity &&
  ...
  function handleInternalDashboardLinkClick(event: MouseEvent<HTMLElement>) {
    ...
    const parsed = parseDashboardSelectionFromUrl(
      `http://localhost${hrefForParsing}`,
    );
    if (!parsed) return;
    ...
    const orderedSources: SearchResultSource[] =
      parsed.source === "sec" ? ["sec", "finra"] : ["finra", "sec"];
  ...
  function extractPayloadFromDetail(detail: any, source: SearchResultSource) {
    ...
    const hasOrphan = Boolean(
      detail?.orphan && typeof detail.orphan === "object",
    );
    const candidate =
      source === "finra"
  ...
  function resolveOrderedSourcesFromDetail(
    detail: any,
    requestedSource: SearchResultSource,
    declaredSources: SearchResultSource[],
  ): SearchResultSource[] {
    const base = [requestedSource, ...declaredSources].filter(
      (entry, index, arr) =>
        (entry === "finra" || entry === "sec") && arr.indexOf(entry) === index,
    );
    const hasFinraData =
      detail?.hasFinraData === true ||
      Boolean(detail?.sources?.finra) ||
      Boolean(detail?.finraNode?.bccontent || detail?.bccontent);
    const hasSecData =
      detail?.hasSecData === true ||
      Boolean(detail?.sources?.sec) ||
      Boolean(detail?.finraNode?.iacontent || detail?.iacontent);
  ...
  async function loadQueueCardsFromRedis(filter = "") {
    try {
      ...
      setQueueMetaStats({
        ...
        ...(payload?.filteredTotalCount != null
          ? { filteredTotalCount: Number(payload.filteredTotalCount || 0) }
          : {}),
        ...(payload?.sourceMode
          ? { sourceMode: String(payload.sourceMode) }
          : {}),
        ...(payload?.persistenceNotice !== undefined
          ? {
              persistenceNotice: payload.persistenceNotice
                ? String(payload.persistenceNotice)
                : null,
            }
          : {}),
        ...(payload?.inventoryTotals
          ? { inventoryTotals: payload.inventoryTotals }
          : {}),
      });
    } catch {
  ...
  function addCardsToLocalHistory(results: any[]) {
    ...
    for (const r of results) {
      ...
      const entity: "individual" | "firm" =
        String(r?.type || "").toLowerCase() === "firm" ? "firm" : "individual";
      const source = String(
        r?.source || "",
      ).toLowerCase() as SearchResultSource;
      if (!/^\d{1,10}$/.test(id)) continue;
      ...
      if (!existing.name && r?.name)
        existing.name = String(r.name).trim() || undefined;
      map.set(key, existing);
    }
    ...
    setLocalHistory((prev) => {
      ...
      const combined = [...newEntries, ...prevFiltered].slice(
        0,
        LOCAL_HISTORY_MAX,
      );
      try {
  ...
  async function fetchMergedDetail(card: QueueCard) {
    ...
    const route =
      card.entity === "firm"
        ? `/api/finra/firm/${card.id}?merged=1&includeConnections=1`
        : `/api/finra/individual/${card.id}?merged=1&includePrevious=true`;
    try {
  ...
  function applyFirmConnectionsToState(
    ...
    if (cached && typeof cached === "object") {
      for (const target of [
        cached,
        cached?.merged,
        cached?.sources?.finra,
        cached?.sources?.sec,
        cached?.finraNode,
      ]) {
        if (target && typeof target === "object") {
          const merged = mergeFirmConnectionLists([
            target.currentConnections,
            target.previousConnections,
            currentConnections,
            previousConnections,
          ]);
    ...
    setMainJson((prev) => {
      if (!prev) return prev;
      const prevCrd = String(
        prev?.basicInformation?.firmId || prev?.firmId || prev?.id || "",
      ).trim();
      if (prevCrd && prevCrd !== firmId) return prev;
      const merged = mergeFirmConnectionLists([
        prev.currentConnections,
        prev.previousConnections,
        currentConnections,
        previousConnections,
      ]);
      return {
  ...
  function connectionsFromExpandPayload(
    firmId: string,
    data: any,
  ): { currentConnections: any[]; previousConnections: any[] } | null {
    ...
    for (const link of links) {
      ...
      const isAssociatedFirm =
        relationship === "associated" ||
        relationship === "previously_associated";
      const isControl =
      ...
      const crd = String(
        other?.crd || otherId.replace(/^(?:person|individual|firm)[:_]/, ""),
      ).trim();
      if (!crd || !/^\d{1,10}$/.test(crd)) continue;

      const firmNode = nodeById.get(firmNodeId) || {};
      const isFirmTerminated =
        /inactive|terminated|revoked|suspended|withdrawn|ceased/i.test(
          String(
            firmNode.firmStatus || firmNode.basicInformation?.firmStatus || "",
          ),
        );
      const isCurrent = isFirmTerminated
        ? false
        : isControl
          ? true
          : relationship === "previous_employed_by" ||
              relationship === "previously_associated"
            ? false
            : link?.isCurrent !== undefined
              ? Boolean(link.isCurrent)
              : !String(
                  link?.endDate || link?.registrationEndDate || "",
                ).trim();
      ...
      const entry = isFirmNode
        ? {
            firmId: crd,
            name: String(other?.label || other?.name || `Firm ${crd}`).trim(),
            relationship: isCurrent
              ? "Associated firm"
              : "Previously associated firm",
            startDate:
              link?.startDate || link?.registrationBeginDate || undefined,
            endDate: isCurrent
              ? undefined
              : link?.endDate || link?.registrationEndDate || undefined,
            isCurrent,
          }
        : {
            individualId: crd,
            name: String(other?.label || other?.name || `Person ${crd}`).trim(),
            relationship: isControl
              ? "Control"
              : isCurrent
      ...
      try {
        ...
        try {
          const expandRes = await fetch(
            `/api/finra/expand/${encodeURIComponent(`firm:${firmId}`)}?hops=1`,
            {
              method: "GET",
              headers: { Accept: "application/json" },
              cache: "default",
          ...
          if (expandRes.ok) {
            ...
            if (fromExpand) {
              applyFirmConnectionsToState(
                firmId,
                fromExpand.currentConnections,
                fromExpand.previousConnections,
              );
            }
          }
        } catch (expandErr) {
        ...
        const response = await fetch(
          `/api/finra/firm/${encodeURIComponent(firmId)}/connections`,
          {
            method: "GET",
            headers: { Accept: "application/json" },
            cache: "default",
        ...
        setConnectionsLoadingFirmId((current) =>
          current === firmId ? null : current,
        );
      }
    })();
  ...
  async function loadQueueSourceJson(
    card: QueueCard,
    source: SearchResultSource,
  ) {
    ...
    if (outgoingScrollKey && dashboardContentRef.current) {
      dashboardScrollPositionsByRecord.set(
        outgoingScrollKey,
        dashboardContentRef.current.scrollTop,
      );
    }
    ...
    setCodeBlock("");
    try {
      let orderedSources: SearchResultSource[] = [
        source,
        ...card.sources
      ...
      orderedSources = resolveOrderedSourcesFromDetail(
        mergedDetail,
        source,
        orderedSources,
      );
      ...
      if (!payload) {
        ...
        const mergedHasAnySource = Boolean(
          mergedDetail?.sources?.finra ||
          mergedDetail?.sources?.sec ||
          mergedDetail?.finraNode ||
          mergedDetail?.merged,
        );
        ...
        if (!payload) {
          ...
          const refreshedItems = Array.isArray(refreshPayload?.results)
          ...
          if (refreshedItems.length > 0) {
            ...
            for (const candidateSource of orderedSources) {
              payload = extractPayloadFromDetail(
                refreshedDetail,
                candidateSource,
              );
              if (payload) {
      ...
      if (
        ...
        (card.entity === "individual"
          ? hasIndividualSourceCoverage(payload, "finra")
          : hasFirmSourceCoverage(payload, "finra"))
      ) {
      ...
      if (
        ...
        (card.entity === "individual"
          ? hasIndividualSourceCoverage(payload, "sec")
          : hasFirmSourceCoverage(payload, "sec"))
      ) {
        detectedSourcesSet.add("sec");
      }
      const isOrphanPayload = Boolean(
        payload?.orphan && typeof payload.orphan === "object",
      );
      ...
      if (!isOrphanPayload && detectedSourcesSet.size === 0 && resolvedSource)
        detectedSourcesSet.add(resolvedSource);
      ...
      const detailName = extractEntityDetailFromPayload(
        payload,
        card.entity,
        card.id,
      )?.name;
      const computedDisplayName = getRecordDisplayName(
      ...
      const resolvedRecordName =
        toText(card.name) ||
        (toText(detailName) && !looksLikeGenericEntityLabel(detailName)
          ? toText(detailName)
          : "") ||
        (toText(computedDisplayName) &&
        !looksLikeGenericEntityLabel(computedDisplayName)
          ? toText(computedDisplayName)
          : "") ||
        resolveEntityNodeLabel(
  ...
  async function runAction(
    ...
    if (action === "fetch-crds") {
      ...
      while (queue.length > 0) {
        ...
        setQueueRunItems((prev) =>
          prev.map((entry) =>
            entry.query === query
              ? { ...entry, status: "running", elapsedSec: 0 }
              : entry,
          ),
        );
        ...
        try {
          ...
          for (const [resultIndex, r] of results.entries()) {
            ...
            newLogs.push({
              id: createQueueTerminalLogId(
                `result-${String(r.source)}-${String(r.crd)}`,
                itemsProcessed,
                resultIndex + 1,
              ),
              text: msg,
          ...
          if (
            errText.includes("no-valid-crds") ||
            errText.includes("No valid CRDs")
          ) {
            setTerminalLogs((prev) => [
              ...prev,
    ...
    try {
      ...
      const timeoutId = setTimeout(
        () => controller.abort("dashboard-request-timeout"),
        15 * 60 * 1000,
      );
  ...
  async function runRedisSearch() {
    ...
    try {
      const [finraIndividualRes, finraFirmRes, secIndividualRes, secFirmRes] =
        await Promise.all([
          fetch(
            `/api/finra/search?type=individual&query=${encodeURIComponent(query)}&rows=8`,
          ),
          fetch(
            `/api/finra/search?type=firm&query=${encodeURIComponent(query)}&rows=8`,
          ),
          fetch(
            `/api/finra/sec-search?query=${encodeURIComponent(query)}&rows=8`,
          ),
          fetch(
            `/api/finra/sec-search-firm?query=${encodeURIComponent(query)}&rows=8`,
          ),
        ]);
      ...
      const getItems = (payload: any) =>
        Array.isArray(payload?.results)
          ? payload.results
          : Array.isArray(payload?.currentPage)
            ? payload.currentPage
            : Array.isArray(payload?.hits?.hits)
              ? payload.hits.hits.map((hit: any) => hit?._source ?? hit)
              : [];
      ...
      const finraIndividuals = normalize(
        getItems(finraIndividualJson),
        "finra",
        "individual",
      );
      const finraFirms = normalize(getItems(finraFirmJson), "finra", "firm");
      const secIndividuals = normalize(
        getItems(secIndividualJson),
        "sec",
        "individual",
      );
      ...
      const skippedTotal =
        finraIndividuals.skipped +
        finraFirms.skipped +
        secIndividuals.skipped +
        secFirms.skipped;
      if (skippedTotal > 0) {
        console.warn(
          "[dashboard] skipped search records due to missing CRD or corrupt payload",
          {
            query,
            skippedTotal,
          },
      ...
      const combinedCards = [
        ...finraIndividuals.cards,
        ...finraFirms.cards,
        ...secIndividuals.cards,
        ...secFirms.cards,
      ];
      ...
      void Promise.all(
        combinedCards
          ...
          .map((card) => hydrateSearchResultCard(card)),
      ).then((hydratedCards) => {
        setSearchResults((current) => {
          const byKey = new Map(
            hydratedCards.map((card) => [
              `${card.entity}:${card.id}:${card.source}`,
              card,
            ]),
          );
          return current.map(
            (card) =>
              byKey.get(`${card.entity}:${card.id}:${card.source}`) || card,
          );
        });
      });
    } catch (error: any) {
  ...
  function renderJsonTree(node: any, depth = 0) {
    ...
    if (node.type === "primitive") {
      return (
        <div className={styles.jsonTreeLeaf} style={{ marginLeft: depth * 12 }}>
    ...
    return (
      <div className={styles.jsonTreeGroup} style={{ marginLeft: depth * 10 }}>
        {node.key && (
          <div className={styles.jsonTreeGroupHeader}>{node.key}</div>
        )}
        {node.children?.map((child: any, index: number) => (
          <div key={`${child.key || "child"}-${index}`}>
            {renderJsonTree(child, depth + 1)}
          </div>
        ))}
      </div>
    );
  ...
  function highlightSearchMatch(text: string, query: string): React.ReactNode {
    ...
    const tokens = Array.from(
      new Set(trimmedQuery.split(/\s+/).filter((token) => token.length >= 2)),
    ...
    return parts.map((part, index) =>
      pattern.test(part) ? (
        <mark key={index} className={styles.searchResultHighlight}>
          {part}
        </mark>
      ) : (
  ...
  function renderSearchResult(card: SearchResultCard, index: number) {
    ...
    const rowAddress =
      card.address || card.detail || "No address/details in cached index";
    const otherNamesText =
      card.otherNames && card.otherNames.length > 0
        ? `aka ${card.otherNames.join(", ")}`
        : "";
    const isSelected =
      currentRecordId === card.id && currentRecordEntity === card.entity;
          ...
          <div className={styles.searchResultRow}>
            <span className={styles.searchResultNameCell}>
              <span className={styles.searchResultName}>
                {highlightSearchMatch(card.label, searchQuery)}
              </span>
              {otherNamesText && (
                <span className={styles.searchResultOtherNames}>
                  {highlightSearchMatch(otherNamesText, searchQuery)}
                </span>
              )}
            </span>
            <span className={styles.searchResultCrd}>CRD #{card.id}</span>
            <span className={styles.searchResultAddress}>
              {highlightSearchMatch(rowAddress, searchQuery)}
            </span>
  ...
  return (
    <div className={styles.page}>
      <header className="fg-header">
        <div className="fg-header-bar">
          <div className="fg-header-brand">
            <h1 className="fg-title" style={{ fontSize: "14px" }}>
              FINRA/SEC
            </h1>
          </div>
      ...
      <div
        className={`${styles.layout} ${!newCrdsOpen ? styles.layoutRightHidden : ""}`}
      >
            ...
            >
              {crawlProgress && crawlProgress.active && (
                <div className={styles.crawlBanner}>
                  <div>
                    <strong>Sequential Crawl:</strong> {crawlProgress.current} /{" "}
                    {crawlProgress.total}
                  ...
                  {recordViewLoading ? (
                    ...
                    <>
                      <div className={styles.recordHeaderRow}>
                        ...
                        >
                          {currentRecordEntity
                            ? String(currentRecordEntity).toUpperCase()
                            : "UNKNOWN"}
                        </span>
                        {currentRecordId && (
                        ...
                        {detailedMainRecord?.hasSecData && (
                          <span className={styles.tagSec}>SEC</span>
                        )}
                        ...
                        {currentRecordEntity === "firm" &&
                          ...
                          (() => {
                            const bi =
                              detailedMainRecord.basicInformation as Record<
                                string,
                                unknown
                              >;
                            const facts: Array<{
                            ...
                            return (
                              <div className={styles.quickFactsPanel}>
                                ...
                                {facts.map((fact) => (
                                  ...
                                  >
                                    <span className={styles.quickFactsLabel}>
                                      {fact.label}
                                    </span>
                                    <span className={styles.quickFactsValue}>
                                      {fact.value}
                                    </span>
                                  </div>
                                ))}
                      ...
                      <h2 className={styles.recordTitle}>
                        {orphanRecord
                          ? currentRecordEntity === "firm"
                            ? formatFirmName(
                                orphanRecord.firmName || mainJsonLabel,
                              )
                            : formatPersonName(
                                orphanRecord.name || mainJsonLabel,
                              )
                          : mainJsonLabel}
                      </h2>
              ...
              {syncBannerText && (
                <div className={styles.statusLine}>{syncBannerText}</div>
              )}
              ...
              {hasCurrentRecord &&
                ...
                mainViewMode === "json" && (
                  <div className={styles.jsonPanel}>
                    ...
                    {jsonTree ? (
                      <div className={styles.jsonTreeList}>
                    ...
                    {orphanRecord ? (
                      (() => {
                        ...
                        const parentType = String(
                          orphanRecord.parentType ||
                            (isFirmOrphan ? "individual" : "firm"),
                        ).toLowerCase();
                        ...
                        const parentLabel = parentIsIndividual
                          ? "Individual"
                          : "Firm";
                        return (
                          <>
                            <div className={styles.detailList}>
                              ...
                              {orphanRecord.mailingAddress && (
                                <div className={styles.detailTextRow}>
                                  ...
                                  {formatAddress(orphanRecord.mailingAddress)}
                                </div>
                              )}
                              {orphanRecord.phone && (
                                <div className={styles.detailTextRow}>
                              ...
                              <div className={styles.detailList}>
                                {!isFirmOrphan && orphanRecord.name && (
                                  <div className={styles.detailRow}>
                                    <div className={styles.detailTextRow}>
                                      <strong>Name:</strong>{" "}
                                      {formatPersonName(orphanRecord.name)}
                                    </div>
                                  </div>
                                )}
                                <div className={styles.detailRow}>
                                ...
                                {orphanRecord.position && (
                                  <div className={styles.detailRow}>
                                    <div className={styles.detailTextRow}>
                                      ...
                                      {formatUiText(orphanRecord.position)}
                                    </div>
                                  </div>
                                )}
                            ...
                            <div className={styles.orphanNoticeAlert}>
                              {isFirmOrphan ? (
                                <>
                                  No independent BrokerCheck/SEC record exists
                                  for Firm CRD {currentRecordId}. This firm was
                                  scraped from{" "}
                                  <Link
                                    href={parentDashboardHref}
                                    className={styles.detailInlineTag}
                                  ...
                                  {orphanRecord.position
                                    ? ` as "${formatUiText(orphanRecord.position)}"`
                                    : ""}
                                  , and has no live CRD of its own.
                                </>
                              ) : (
                                <>
                                  No independent BrokerCheck/SEC record exists
                                  for CRD {currentRecordId}. This person was
                                  scraped from{" "}
                                  <Link
                                    href={parentDashboardHref}
                                    className={styles.detailInlineTag}
                                  ...
                                  's own detail record as "
                                  {formatUiText(orphanRecord.position)}", and
                                  has no live CRD of its own.
                                </>
                              )}
                            </div>
                          </>
                      ...
                      <>
                        {detailedMainRecord.otherNames.length > 0 && (
                          <div className={styles.detailOtherNamesBlock}>
                            <div className={styles.detailOtherNamesHeading}>
                              OTHER NAMES
                            </div>
                            <div
                            ...
                            >
                              {detailedMainRecord.otherNames.map((name) => {
                                const formatted = formatOtherName(
                                  name,
                                  currentRecordEntity === "firm",
                                );
                                const isHighlighted =
                                  currentRecordEntity === "firm" &&
                                  highlightOtherNameFromNav &&
                                  String(formatted).trim().toLowerCase() ===
                                    highlightOtherNameFromNav;
                                return (
                                  <span
                                    key={name}
                        ...
                        <section className={styles.detailSection}>
                          ...
                          <div className={styles.detailLinkRow}>
                            ...
                            {detailedMainRecord.documentLinkCards.map(
                              (link) => (
                                <a
                                  key={link.href}
                                  href={link.href}
                            ...
                            <div className={styles.detailGrid}>
                              <div className={styles.detailGridCard}>
                                <div className={styles.detailRowMain}>
                                  <span className={styles.detailRowName}>
                                    CRS Document (
                                    {detailedMainRecord.crs.crsType || "PDF"})
                                  </span>
                                ...
                                {detailedMainRecord.crs.fileId && (
                                  <div className={styles.detailRowMeta}>
                                    ID: {detailedMainRecord.crs.fileId}
                                  </div>
                                )}
                              </div>
                            </div>
                              ...
                              {Object.entries(
                                ...
                                >
                                  <div className={styles.detailRowMain}>
                                    ...
                                    <span className={styles.detailInlineTag}>
                                      {String(val)}
                                    </span>
                                  </div>
                                </div>
                        ...
                        {detailedMainRecord.currentEmployment.length > 0 && (
                          <section className={styles.detailSection}>
                            <h4 className={styles.detailSectionTitle}>
                              Current Employment (
                              {detailedMainRecord.currentEmployment.length})
                            </h4>
                            <div className={styles.detailList}>
                              {detailedMainRecord.currentEmployment.map(
                                (row, idx) => {
                                  const crd = pickFirstValidCrd(
                                    row.crdNumber,
                                    row.crd,
                                  ...
                                  const dateStr = startDate
                                    ? `Since ${startDate}`
                                    : "";
                                  const metaParts = [address, dateStr].filter(
                                    Boolean,
                                  );
                                  const metaLine =
                                    metaParts.length > 0
                                      ...
                                      : formatUiText(
                                          pickFirstNonEmpty(
                                            row.position,
                                            row.currentRegistration,
                                            row.status,
                                          ),
                                        );
                                  const rowName = resolveEntityNodeLabel(
                                    row,
                                    "firm",
                                    crd,
                                    idx,
                                  ...
                                  const otherNames =
                                    (crd &&
                                      employmentFirmOtherNames[String(crd)]) ||
                                    [];
                                  ...
                                  const content = (
                                    <>
                                      <div className={styles.detailRowMain}>
                                        <div
                                          className={
                                            styles.employmentRowNameWrap
                                          }
                                        >
                                          <span
                                            className={styles.detailRowName}
                                          >
                                            {rowName}
                                          </span>
                                          {otherNames.length > 0 && (
                                            <span
                                              className={
                                                styles.employmentRowOtherNames
                                              }
                                            >
                                              aka{" "}
                                              {otherNames
                                                .map((n) =>
                                                  formatOtherName(n, true),
                                                )
                                                .join(", ")}
                                            </span>
                                          )}
                                  ...
                                  if (crd) {
                                    const rowNameText =
                                      typeof rowName === "string"
                                        ? rowName
                                        : "";
                                    const href = rowNameText
                                      ? `/dashboard/firm/${crd}?fromName=${encodeURIComponent(rowNameText)}`
                                      : `/dashboard/firm/${crd}`;
                                    return (
                                      <Link
                                        href={href}
                        ...
                        {detailedMainRecord.previousEmployment.length > 0 && (
                          <section className={styles.detailSection}>
                            <h4 className={styles.detailSectionTitle}>
                              Previous Employment (
                              {detailedMainRecord.previousEmployment.length})
                            </h4>
                            <div className={styles.detailList}>
                              {detailedMainRecord.previousEmployment.map(
                                (row, idx) => {
                                  const crd = pickFirstValidCrd(
                                    row.crdNumber,
                                    row.crd,
                        ...
                        {detailedMainRecord?.disclosureSummary &&
                          detailedMainRecord.disclosureSummary.length > 0 && (
                            <section
                              className={styles.detailSection}
                            ...
                            >
                              ...
                              <div className={styles.detailGrid}>
                                {detailedMainRecord.disclosureSummary.map(
                                  (d: any) => (
                                    <div
                              ...
                              {detailedMainRecord.rawDisclosures &&
                                detailedMainRecord.rawDisclosures.some(
                                  (dis: any) =>
                                    dis.disclosureDetail &&
                                    Object.keys(dis.disclosureDetail).length >
                                      0,
                                ) && (
                                  <div
                                    style={{
                                  ...
                                  >
                                    {detailedMainRecord.rawDisclosures
                                      .filter(
                                        (dis: any) =>
                                          dis.disclosureDetail &&
                                          Object.keys(dis.disclosureDetail)
                                            .length > 0,
                                      )
                                      .map((dis: any, idx: number) => {
                                        const dtype = String(
                                          dis.disclosureType || dis.type || "",
                                        ).trim();
                                        const ddate = String(
                                          dis.eventDate || dis.date || "",
                                        ).trim();
                                        const dres = String(
                                          dis.disclosureResolution ||
                                            dis.resolution ||
                                            "",
                                        ).trim();
                                        ...
                                        const sanctionText = String(
                                          dd["Sanctions"] ||
                                            dd["sanctions"] ||
                                            "",
                                        ).trim();
                                        const sanctionDetails = Array.isArray(
                                          dd["SanctionDetails"] ||
                                            dd["Sanction Details"],
                                        )
                                          ? dd["SanctionDetails"] ||
                                        ...
                                        const comments = Array.isArray(
                                          ...
                                          : brokerCommentRaw
                                            ? [brokerCommentRaw]
                                            : [];
                                        const settlementAmt = String(
                                          dd["Settlement Amount"] ||
                                            dd["settlementAmount"] ||
                                            "",
                                        ).trim();
                                        const docketFDA = String(
                                          dd["DocketNumberFDA"] || "",
                                        ).trim();
                                        const docketAAO = String(
                                          dd["DocketNumberAAO"] || "",
                                        ).trim();
                                        const arbDocket = String(
                                          dd["arbitrationDocketNumber"] || "",
                                        ).trim();
                                        ...
                                        const extraDetailRows = Object.entries(
                                          dd,
                                        )
                                          .map(([key, value]) => ({
                                            key,
                                        ...
                                        return (
                                          ...
                                          >
                                            <div className="fg-dis-header">
                                              <span className="fg-dis-type">
                                                {dtype}
                                              </span>
                                              {ddate && (
                                                <span className="fg-dis-date">
                                                  {ddate}
                                                </span>
                                              )}
                                              {dres && (
                                                <span
                                                  className={`fg-dis-res ${/final|settled/i.test(dres) ? "final" : "pending"}`}
                                                >
                                                  {dres}
                                                </span>
                                              )}
                        ...
                        {detailedMainRecord.registrations &&
                          Object.keys(detailedMainRecord.registrations).length >
                            0 && (
                            <section className={styles.detailSection}>
                              <h4 className={styles.detailSectionTitle}>
                                Registrations
                              ...
                              {detailedMainRecord.registrations.stateList
                                ?.length > 0 && (
                                <div className={styles.registeredStatesWrap}>
                                  <h5 className={styles.registeredStatesTitle}>
                                    Registered States (
                                    {
                                      detailedMainRecord.registrations.stateList
                                        .length
                                  ...
                                  <div className={styles.registeredStatesList}>
                                    {detailedMainRecord.registrations.stateList.map(
                                      (st: any, idx: number) => (
                                        <span
                                          key={`state-${idx}`}
                        ...
                        {detailedMainRecord.directOwners?.length > 0 && (
                          <section className={styles.detailSection}>
                            <h4 className={styles.detailSectionTitle}>
                              ...
                              {detailedMainRecord.directOwners.length})
                            </h4>
                            <div className={styles.detailList}>
                              {detailedMainRecord.directOwners.map(
                                (row, idx) => {
                                  const crd = pickFirstValidCrd(
                                    row.crdNumber,
                                    row.crd,
                                  ...
                                  const name = resolveEntityNodeLabel(
                                    row,
                                    "individual",
                                    crd,
                                    idx,
                                  );
                                  const position = formatUiText(
                                    pickFirstNonEmpty(row.position, row.title),
                                  );
                                  const acquiredDate = pickFirstNonEmpty(
                                    row.acquiredDate,
                                    row.dateAcquired,
                                    row.startDate,
                                  );
                        ...
                        {detailedMainRecord.indirectOwners?.length > 0 && (
                          <section className={styles.detailSection}>
                            <h4 className={styles.detailSectionTitle}>
                              Indirect Owners (
                              {detailedMainRecord.indirectOwners.length})
                            </h4>
                            <div className={styles.detailList}>
                              {detailedMainRecord.indirectOwners.map(
                                (row, idx) => {
                                  const crd = pickFirstValidCrd(
                                    row.crdNumber,
                                    row.crd,
                        ...
                        {detailedMainRecord.stateExams.length > 0 && (
                          <section className={styles.detailSection}>
                            <h4 className={styles.detailSectionTitle}>
                              🏛️ STATE EXAM CATEGORY (
                              {detailedMainRecord.stateExams.length})
                            </h4>
                              ...
                              {detailedMainRecord.stateExams.map((row, idx) => (
                                ...
                                >
                                  <div className={styles.detailExamTop}>
                                    <span
                                      className={styles.detailExamBadgeState}
                                    >
                                      {pickFirstNonEmpty(
                                        row.examCategory,
                                        row.examCode,
                                    ...
                                    {pickFirstNonEmpty(
                                      ...
                                      <span className={styles.detailExamDate}>
                                        📅{" "}
                                        {pickFirstNonEmpty(
                                          row.examTakenDate,
                                          row.dateTaken,
                                  ...
                                  <div className={styles.detailExamName}>
                                    {pickFirstNonEmpty(
                                      row.examName,
                                      row.description,
                                      row.categoryName,
                                  ...
                                  {pickFirstNonEmpty(
                                    ...
                                    <div className={styles.detailExamScope}>
                                      Scope:{" "}
                                      {pickFirstNonEmpty(
                                        row.examScope,
                                        row.scope,
                        ...
                        {detailedMainRecord.productExams.length > 0 && (
                          <section className={styles.detailSection}>
                            <h4 className={styles.detailSectionTitle}>
                              📜 PRODUCT EXAM CATEGORY (
                              {detailedMainRecord.productExams.length})
                            </h4>
                            <div className={styles.detailExamGrid}>
                              {detailedMainRecord.productExams.map(
                                (row, idx) => (
                                  <div
                                    key={`prod-exam-${idx}`}
                        ...
                        {detailedMainRecord.principalExams.length > 0 && (
                          <section className={styles.detailSection}>
                            <h4 className={styles.detailSectionTitle}>
                              👔 PRINCIPAL EXAM CATEGORY (
                              {detailedMainRecord.principalExams.length})
                            </h4>
                            <div className={styles.detailExamGrid}>
                              {detailedMainRecord.principalExams.map(
                                (row, idx) => (
                                  <div
                                    key={`princ-exam-${idx}`}
                                  ...
                                  >
                                    <div className={styles.detailExamTop}>
                                      <span
                                        className={
                                          styles.detailExamBadgePrincipal
                                        }
                                      >
                                        {pickFirstNonEmpty(
                                          row.examCategory,
                        ...
                        {detailedMainRecord.brochureCards.length > 0 && (
                          <section className={styles.detailSection}>
                            <h4 className={styles.detailSectionTitle}>
                              Brochures (
                              {detailedMainRecord.brochureCards.length})
                            </h4>
                            {detailedMainRecord.brochuresPart2Exempt && (
                              ...
                              >
                                <strong>Part 2 Exempt:</strong>{" "}
                                {detailedMainRecord.brochuresPart2Exempt}
                              </div>
                            )}
                            <div className={styles.detailGrid}>
                              {detailedMainRecord.brochureCards.map(
                                (item, idx) => (
                                  <div
                                    key={`brochure-${idx}`}
                                    className={styles.detailGridCard}
                        ...
                        {detailedMainRecord.noticeFilingCards.length > 0 && (
                          <section className={styles.detailSection}>
                            <h4 className={styles.detailSectionTitle}>
                              Notice Filings (
                              {detailedMainRecord.noticeFilingCards.length})
                            </h4>
                            <div className={styles.detailGrid}>
                              {detailedMainRecord.noticeFilingCards.map(
                                (item, idx) => (
                                  <div
                        ...
                        {detailedMainRecord.jurisdictionCards.length > 0 && (
                          <section className={styles.detailSection}>
                            <h4 className={styles.detailSectionTitle}>
                              Registered States (
                              {detailedMainRecord.jurisdictionCards.length})
                            </h4>
                            <div className={styles.detailTagList}>
                              {detailedMainRecord.jurisdictionCards.map(
                                (item, idx) => (
                                  <span
                                    key={`${item.title}-${idx}`}
                          ...
                          <section className={styles.detailSection}>
                            ...
                            <div className={styles.detailTagList}>
                              {detailedMainRecord.registeredSros.map((tag) => (
                                <span key={tag} className={styles.detailTagSro}>
                                  {tag}
                                </span>
                              ))}
                            </div>
                        ...
                        connectionsLoadingFirmId === currentRecordId
                          ? null
                          : (() => {
                              const matchesConnectionsFilter = (item: {
                                title?: string;
                                subtitle?: string;
                                meta?: string;
                                crd?: string;
                              ...
                              const filteredCurrentConnectionCards =
                                detailedMainRecord.currentConnectionCards.filter(
                                  matchesConnectionsFilter,
                                );
                              const filteredPreviousConnectionCards =
                                detailedMainRecord.previousConnectionCards.filter(
                                  matchesConnectionsFilter,
                                );
                                ...
                                <>
                                  {(detailedMainRecord.currentConnectionCards
                                    ...
                                    <FilterTagsInput
                                      ...
                                      onLiveTextChange={
                                        setConnectionsFilterQuery
                                      }
                                      placeholder="Filter connections…"
                                    />
                                  )}

                                  {filteredCurrentConnectionCards.length >
                                    0 && (
                                    <section className={styles.detailSection}>
                                      <h4 className={styles.detailSectionTitle}>
                                        Current Connections (
                                        {
                                          detailedMainRecord
                                            .currentConnectionCards.length
                                        }
                                        )
                                      </h4>
                                      <div className={styles.detailList}>
                                        {filteredCurrentConnectionCards.map(
                                          (item, idx) => {
                                            const content = (
                                              <>
                                                <div
                                                ...
                                                >
                                                  <span
                                                    className={`${styles.detailRowName} ${styles.currentConnectionName}`}
                                                  >
                                                    {item.title}
                                                  </span>
                                                  {item.crd && (
                                                  ...
                                                  {item.statusTag && (
                                                    <span
                                                      className={`${styles.detailInlineTag} ${styles.currentConnectionStatusTag} ${
                                                        item.statusTag ===
                                                        "BD Stub Only"
                                                          ? styles.currentConnectionStatusTagStub
                                                          : item.statusTag ===
                                                              "Inactive"
                                                ...
                                                {item.otherNames &&
                                                  item.otherNames.length >
                                                    0 && (
                                                    <div
                                                      className={`${styles.detailRowMeta} ${styles.currentConnectionOtherNames}`}
                                                    >
                                                      Also known as:{" "}
                                                      {item.otherNames
                                                        .map((n) =>
                                                ...
                                                {item.subtitle && (
                                                  <div
                                                    className={`${styles.detailRowMeta} ${styles.currentConnectionMeta}`}
                                                  >
                                                    {item.subtitle}
                                                  </div>
                                                )}
                                  ...
                                  {filteredPreviousConnectionCards.length >
                                    0 && (
                                    <section className={styles.detailSection}>
                                      <h4 className={styles.detailSectionTitle}>
                                        ...
                                        {
                                          detailedMainRecord
                                            .previousConnectionCards.length
                                        }
                                        )
                                      </h4>
                                      <div className={styles.detailList}>
                                        {filteredPreviousConnectionCards.map(
                                          (item, idx) => {
                                            const content = (
                                              <>
                                                <div
                                                ...
                                                >
                                                  <span
                                                    className={`${styles.detailRowName} ${styles.previousConnectionName}`}
                                                  >
                                                    {item.title}
                                                  </span>
                                                  {item.crd && (
                                                ...
                                                {item.otherNames &&
                                                  item.otherNames.length >
                                                    0 && (
                                                    <div
                                                      className={`${styles.detailRowMeta} ${styles.previousConnectionOtherNames}`}
                                                    >
                                                      Also known as:{" "}
                                                      {item.otherNames
                                                        .map((n) =>
                                                ...
                                                {item.subtitle && (
                                                  <div
                                                    className={`${styles.detailRowMeta} ${styles.previousConnectionMeta}`}
                                                  >
                                                    {item.subtitle}
                                                  </div>
                                                )}
                      ...
                      <div className={styles.readableCardEmpty}>
                        No readable fields found for this record.
                      </div>
                    )}
                  </div>
                )}
              ...
              {!hasCurrentRecord && (
                <div className={styles.searchSummary}>
                  No node selected yet. Search for a specific CRD below.
                </div>
              )}

              <div
                className={`${styles.searchBarWrap} ${searchPaneOpen ? styles.searchBarWrapExpanded : ""}`}
              >
                <div
                  className={`${styles.searchResultsPane} ${searchPaneOpen ? styles.searchResultsPaneOpen : ""}`}
                >
                  {searchPaneOpen && (
                    <button
                      type="button"
                  ...
                  {searchResults.length > 0 ? (
                    ...
                    <div className={styles.searchResultsEmpty}>
                      No Redis results yet for this query.
                    </div>
                  ) : null}
                </div>
                <div className={styles.searchDock}>
                  <div className={styles.searchDockTitleRow}>
                    <div className={styles.searchTitle}>
                      REDIS SEARCH ({searchResults.length.toLocaleString()})
                    </div>
                      ...
                      {(() => {
                        ...
                        if (
                          ...
                          if (
                            ...
                            host === "finra-data-chart-next-02.vercel.app"
                          ) {
        ...
        <aside className={styles.middlePane}>
          ...
          >
            ...
            <div className={styles.middlePaneActions}>
              <span className={styles.middlePaneCount}>
                {displayCards.length}
              </span>
          ...
          {isSelectionHistoryOpen && (
            ...
            >
              {displayCards.length > 0 ? (
                displayCards.map((card) =>
                  (() => {
                    ...
                    const computedCardName =
                      storedName && !looksLikeGenericEntityLabel(storedName)
                        ? storedName
                        : isActiveRecord && toText(mainJsonLabel)
                          ? toText(mainJsonLabel)
                    ...
                    return (
                      ...
                      >
                        <div className={styles.middlePaneItemTop}>
                          ...
                          <span className={styles.middlePaneItemBadge}>
                            {card.entity === "firm" ? "FIRM" : "IND"}
                          </span>
                          <span className={styles.middlePaneItemName}>
                            {computedCardName}
                          </span>
                        ...
                        <div className={styles.middlePaneItemMeta}>
                          CRD #{card.id}
                        </div>
                      </button>
                    );
                ...
                <div className={styles.middlePaneEmpty}>
          ...
          <div
            ...
            style={{
              ...
              borderTop: isSelectionHistoryOpen
          ...
          >
            ...
            <div className={styles.middlePaneActions}>
              <span className={styles.middlePaneCount}>
                {graphClickHistory.length}
              </span>
              {isGraphClickHistoryEditMode &&
                selectedGraphClickIndexes.size > 0 && (
                  <button
                    type="button"
                    className={styles.middlePaneClearBtn}
                    onClick={(e) => {
              ...
              {graphClickHistory.length > 0 ? (
                graphClickHistory
                  ...
                  .map(({ entry, originalIdx }) => {
                    const entityForLink =
                      entry.group === "firm" ? "firm" : "individual";
                    const extractedCrd = entry.secondaryId.replace(
                      /[^0-9]/g,
                      "",
                    );
                    const isActiveRecord =
                      currentRecordId === extractedCrd &&
                      currentRecordEntity === entityForLink;
                    const isChecked =
                      selectedGraphClickIndexes.has(originalIdx);
        ...
        >
          ...
          >
            {newCrdsOpen && (
              <>
                <div className={styles.rightPaneCountCard}>
                  {uniqueCrdCounts.total.toLocaleString()} unique CRDs saved in
                  Redis
                </div>
                    ...
                    {peopleCrdEntries.length > 0 ? (
                      peopleCrdEntries.map((entry) => {
                        const isSelected =
                          ...
                          currentRecordEntity === "individual";
                        const { hasFinra, hasSec } = getNewCrdSources(
                          entry,
                          "individual",
                          localHistory,
                        );
                        return (
                          ...
                          >
                            ...
                            <div className={styles.rightPaneItemMeta}>
                              CRD #{entry.id}
                            </div>
                          </button>
                        );
