...
async function getConnectionsFromBrokerIdMirror(
  firmId: string,
  ...
  const [finraCurrentRaw, finraPreviousRaw, secCurrentRaw, secPreviousRaw] =
    await Promise.all([
  ...
  const currentIds = new Set<string>([
    ...parseBrokerIdList(finraCurrentRaw),
    ...parseBrokerIdList(secCurrentRaw),
  ]);
  const previousIds = new Set<string>([
    ...parseBrokerIdList(finraPreviousRaw),
    ...parseBrokerIdList(secPreviousRaw),
  ]);
...
async function persistBrokerIdLists(
  firmId: string,
  ...
  const buckets: Record<
    "finra" | "sec",
    { connected: Set<string>; previous: Set<string> }
  > = {
  ...
  for (const entry of payload.currentConnections || []) {
    ...
    for (const source of evidenceSources(entry))
      buckets[source].connected.add(id);
  }
  for (const entry of payload.previousConnections || []) {
    ...
    if (!id || !/^\d{1,10}$/.test(id)) continue;
    for (const source of evidenceSources(entry))
      buckets[source].previous.add(id);
  }
...
async function mergeBrokerIdListsAdditive(
  firmId: string,
  payload: FirmConnectionsPayload,
) {
  const redis = getRedisClient();
  ...
  for (const entry of payload.currentConnections || []) {
    ...
    if (!id || !/^\d{1,10}$/.test(id)) continue;
    for (const source of evidenceSources(entry))
      newlyValidated[source].connected.add(id);
  }
  for (const entry of payload.previousConnections || []) {
    if (!hasValidatedFirmConnectionEvidence(entry)) continue;
    const id = firstNonEmpty(entry.individualId);
    if (!id || !/^\d{1,10}$/.test(id)) continue;
    for (const source of evidenceSources(entry))
      newlyValidated[source].previous.add(id);
  }
  if (
    !newlyValidated.finra.connected.size &&
    !newlyValidated.finra.previous.size &&
    !newlyValidated.sec.connected.size &&
    !newlyValidated.sec.previous.size
  )
    return;

  for (const source of ["finra", "sec"] as const) {
    const { connected: newConnected, previous: newPrevious } =
      newlyValidated[source];
    if (!newConnected.size && !newPrevious.size) continue;
    try {
      const [existingConnectedRaw, existingPreviousRaw] = await Promise.all([
        redis.get(`${source}:firm:${firmId}_brokers:current`).catch(() => null),
        redis
          .get(`${source}:firm:${firmId}_brokers:previous`)
          .catch(() => null),
      ]);
      const connected = new Set<string>([
        ...parseBrokerIdList(existingConnectedRaw),
        ...newConnected,
      ]);
      const previous = new Set<string>([
        ...parseBrokerIdList(existingPreviousRaw),
        ...newPrevious,
      ]);
      // A person cannot be both current and previous for the same source — current wins.
      for (const id of connected) previous.delete(id);
...
export type IndividualEmploymentFirmIds = {
  current: string[];
  previous: string[];
};
...
export async function syncBrokerIdMirrorForIndividualChange(
  crd: string,
  oldFirmIds: string[],
  newFirmIds: string[],
  ...
  const oldSet = new Set(
    oldFirmIds.map((f) => firstNonEmpty(f)).filter(Boolean),
  );
  const newSet = new Set(
    newFirmIds.map((f) => firstNonEmpty(f)).filter(Boolean),
  );
    ...
    for (const firmId of gained) {
      try {
        const [connectedRaw, previousRaw] = await Promise.all([
          redis
            .get(`${source}:firm:${firmId}_brokers:current`)
            .catch(() => null),
          redis
            .get(`${source}:firm:${firmId}_brokers:previous`)
            .catch(() => null),
...
async function backfillBrokerIdListsIfMissing(
  firmId: string,
  payload: FirmConnectionsPayload,
) {
  const redis = getRedisClient();
  if (!redis) return;
  const checkKeys = [
    `finra:firm:${firmId}_brokers:current`,
    `finra:firm:${firmId}_brokers:previous`,
    `sec:firm:${firmId}_brokers:current`,
    `sec:firm:${firmId}_brokers:previous`,
  ];
  let anyExists = false;
...
export async function getFirmConnectionsFromGraph(
  ...
  // Validation comes exclusively from each individual's own cached current/previous
  // employment history (enrichConnectionEntriesFromIndividualCache below) — the mono graph
  // store's employed_by links were a separate, less-trustworthy artifact that could carry
  // stale/incorrect data forward and have been removed as an evidence source.
  const brokerMirrorEntries = await getConnectionsFromBrokerIdMirror(
    normalizedFirmId,
  ).catch(() => [] as GraphConnectionEntry[]);
  if (brokerMirrorEntries.length) {
    let result = mergeGraphConnectionEntries([brokerMirrorEntries]);
    result = {
      ...result,
      currentConnections: await enrichConnectionEntriesFromIndividualCache(
        result.currentConnections,
        normalizedFirmId,
        redis,
      ),
      previousConnections: await enrichConnectionEntriesFromIndividualCache(
    ...
    result = {
      ...result,
      currentConnections: filterOutDisprovenBrokerMirrorEntries(
        result.currentConnections,
      ),
      previousConnections: filterOutDisprovenBrokerMirrorEntries(
        result.previousConnections,
      ),
    };
    ...
    const backfillFrom = (
      entries: GraphConnectionEntry[],
      sourceById: Map<string, GraphConnectionEntry>,
    ) =>
      entries.map((entry) => {
        ...
        return {
          ...
          otherNames: entry.otherNames?.length
            ? entry.otherNames
            : match.otherNames,
    ...
    if (
      ...
      result = {
        ...
        previousConnections: backfillFrom(
          result.previousConnections,
          localById,
        ),
      };
    ...
    let officialRosterFetched = false;
    {
      ...
      if (official) {
        ...
        result = {
          ...result,
          currentConnections: backfillFrom(
            result.currentConnections,
            officialById,
          ),
          previousConnections: backfillFrom(
    ...
    const stripInternalMarkers = (entries: GraphConnectionEntry[]) =>
      entries.map(
        ({ __employmentChecked, __employmentMatched, ...rest }: any) =>
          rest as GraphConnectionEntry,
      );
    result = {
      ...result,
    ...
    const payload: FirmConnectionsPayload = {
      ...result,
      source: "broker-id-mirror",
    };
