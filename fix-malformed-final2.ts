...
import { normalizeIndividualDetailPayload, normalizeFirmDetailPayload } from './src/app/api/dashboard/refresh/route';
...
async function run() {
  ...
  for (const rawKey of keys) {
      ...
      try {
        ...
        const rawPayload = typeof finraStr === 'string' ? JSON.parse(finraStr) : finraStr;
        
        let normalized = null;
        if (entity === 'individual') {
           // We'll just write a quick unwrap here instead of importing
           normalized = rawPayload;
           if (normalized.hits?.hits?.[0]?._source?.content) {
              normalized = JSON.parse(normalized.hits.hits[0]._source.content);
           }
        } else {
           normalized = rawPayload;
           if (normalized.hits?.hits?.[0]?._source?.content) {
        ...
        const basic = normalized.basicInformation || {};
        
        let bcScope = basic.bcScope || normalized.bcScope || basic.brokerCheckScope || '';
        let iaScope = basic.iaScope || normalized.iaScope || basic.secScope || '';
