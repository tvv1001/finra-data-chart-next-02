...
(async function main(){
  for (let iter = 1; iter <= maxIterations; iter++) {
    ...
    const crawlArgs = ['scripts/parallel_crawler.js', '--concurrency', concurrency, '--delay', delay, '--limit', String(limit)];
    ...
    runAttach('node', ['scripts/build_graph_from_cache.js']);

    console.log('POSTing to /api/finra/recompute-meta to update finra-graph.json');
    runAttach('curl', ['-s', '-X', 'POST', 'http://localhost:3001/api/finra/recompute-meta']);
