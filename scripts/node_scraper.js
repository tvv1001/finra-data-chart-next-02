...
async function fetchJson(url) {
  try { const r = await axios.get(url, { timeout: 20000 }); return r.data; } catch { return null; }
}

async function loadGraph() {
  try { const raw = await fs.readFile(GRAPH_FILE, 'utf-8'); return JSON.parse(raw); } catch { return { nodes: [], links: [], meta: {} }; }
}

async function writeGraph(g) { await fs.writeFile(GRAPH_FILE, JSON.stringify(g, null, 2), 'utf-8'); }
...
async function run(seeds = [], maxDepth = 2) {
  ...
  while (queue.length) {
    ...
    if (id.startsWith('person:')) {
      ...
      for (const e of emps) {
        ...
        if (!seen.has(fidKey) && (depth+1) <= maxDepth) { queue.push(fidKey); depthMap.set(fidKey, depth+1); }
      }
      ...
      for (const o of owners) {
        if (o.ownerFirmId) {
          ...
          if (!seen.has(key) && (depth+1) <= maxDepth) { queue.push(key); depthMap.set(key, depth+1); }
        } else if (o.ownerId) {
  ...
  try { await axios.post(`${BASE_API}/api/finra/graph-append`, { nodes: [], links: [] }, { timeout: 20000 }); } catch {}
...
if (require.main === module) {
  ...
  run(seeds, depth).catch(e => { console.error(e); process.exit(1); });
}
