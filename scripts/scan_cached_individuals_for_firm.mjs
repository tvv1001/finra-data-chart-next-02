...
if (!FIRM_ID) {
	console.error('Usage: node scripts/scan_cached_individuals_for_firm.mjs --firm <firmId>');
	process.exit(2);
}
...
function decompress(v) {
	...
	if (v.startsWith('br:')) return zlib.brotliDecompressSync(Buffer.from(v.slice(3), 'base64')).toString('utf8');
	if (v.startsWith('gz:')) return zlib.gunzipSync(Buffer.from(v.slice(3), 'base64')).toString('utf8');
	return v;
}
...
function extractContent(rawJson) {
	...
	if (rawJson?.basicInformation || rawJson?.currentEmployments || rawJson?.currentIAEmployments) {
		return rawJson;
	}
	return null;
}
...
function employmentArraysReferencingFirm(content, firmId) {
	const currentArrays = [content?.currentEmployments, content?.currentIAEmployments].filter(Array.isArray);
	const previousArrays = [content?.previousEmployments, content?.previousIAEmployments].filter(Array.isArray);
	const matchesFirm = (e) => {
...
function extractName(content) {
	...
	return [bi.firstName, bi.middleName, bi.lastName].filter(Boolean).join(' ') || bi.individualName || '';
}
...
async function main() {
	...
	for (let i = 0; i < allKeys.length; i += BATCH_SIZE) {
		...
		for (let j = 0; j < batch.length; j++) {
			...
			try {
				...
				if (!content) continue;
				const { currentMatch, previousMatch } = employmentArraysReferencingFirm(content, FIRM_ID);
				if (currentMatch) confirmedCurrent.push({ crd, name: extractName(content) });
				else if (previousMatch) confirmedPrevious.push({ crd, name: extractName(content) });
			} catch {
				parseErrors++;
			}
		}
		if (scanned % 5000 < BATCH_SIZE) console.log(`  scanned ${scanned}/${allKeys.length}...`);
	}


	console.log(`\n=== Results for firm ${FIRM_ID} ===`);
	console.log(`Confirmed CURRENT connections (from cached individual records): ${confirmedCurrent.length}`);
	confirmedCurrent.forEach((c) => console.log(`  - ${c.crd}: ${c.name || '(no name)'}`));
	console.log(`\nConfirmed PREVIOUS connections (from cached individual records): ${confirmedPrevious.length}`);
	confirmedPrevious.forEach((c) => console.log(`  - ${c.crd}: ${c.name || '(no name)'}`));
	...
	const confirmedCrds = new Set([...confirmedCurrent, ...confirmedPrevious].map((c) => c.crd));
	const mirrorOnly = [...mirrorCrds].filter((c) => !confirmedCrds.has(c));
	const confirmedNotInMirror = [...confirmedCrds].filter((c) => !mirrorCrds.has(c));
	console.log(`\nFirm's broker-id-mirror total unique CRDs: ${mirrorCrds.size}`);
	console.log(`Mirror CRDs still unconfirmed (no cached individual record proves/disproves them yet): ${mirrorOnly.length}`);
	if (confirmedNotInMirror.length) {
		console.log(`CRDs confirmed via cache but MISSING from the firm's mirror lists (should be added): ${confirmedNotInMirror.length}`);
