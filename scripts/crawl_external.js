...
async function crawl(limit = 500, force = false) {
  ...
  try {
    ...
    for (const f of ext) {
      ...
      if (!j) continue;
      const { firms, inds } = extractIdsFromJson(j);
      for (const id of firms) if (!seenF.has(id)) { seenF.add(id); queueF.push(id); }
      for (const id of inds) if (!seenI.has(id)) { seenI.add(id); queueI.push(id); }
    }
  } catch (e) {}
  ...
  while ((queueF.length || queueI.length) && fetched < limit) {
    ...
    // if looks numeric and length > 1, try both
    const numeric = /^\d+$/.test(id);

    if (numeric) {
      // individual
      if (force || !await exists(path.join(FINRA_DIR, `individual_${id}.json`))) {
        ...
        if (data) {
          ...
          for (const fid of firms) if (!seenF.has(fid)) { seenF.add(fid); queueF.push(fid); }
          for (const iid of inds) if (!seenI.has(iid)) { seenI.add(iid); queueI.push(iid); }
        }
        fetched++;
      ...
      // treat as firm id or query string
...
if (require.main === module) {
  ...
  crawl(limit, force).catch((e) => { console.error(e); process.exit(1); });
}
