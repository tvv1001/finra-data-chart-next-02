...
// Respect global disable switch for any scripts that might call external APIs
assertExternalApisEnabled('build_graph_from_cache.js');
...
async function build(options = {}) {
	...
	// Normalize and attach richer individual details from parsed files (FINRA/SEC raw payloads)
	function normalizeEmploymentRecord(e) {
		...
		const firm_bd_sec_number = String(e.bdSECNumber || e.bd_sec_number || e.bdSecNumber || e.firm_bd_sec_number || e.firm_bd_sec_number || '').trim() || undefined;
		const normalized = {
			firm_id: e.firmId || e.firm_id ? String(e.firmId || e.firm_id) : undefined,
			firm_name: e.firmName || e.firm_name || undefined,
			branch_city: branch.city || branch.city || branch.city || undefined,
			branch_state: branch.state || branch.state || undefined,
	...
	function normalizeIndividualPayload(c) {
		...
		const name = (basic.firstName || basic.name || basic.firmName || '') + (basic.lastName ? ` ${basic.lastName}` : '');
		const currentEmps =
	...
	// Attach normalized details to person nodes when available in parsed files
	try {
		for (const f of parsedFiles) {
			const obj = f.json || f;
			const hits = obj?.hits?.hits || [];
			for (const hit of hits) {
				const src = hit._source || {};
				let crd = src.ind_source_id || (src.person && src.person.crd) || null;
				if (!crd && src.content) {
					try {
						const c = typeof src.content === 'string' ? JSON.parse(src.content) : src.content;
						crd = c?.basicInformation?.crd || c?.basicInformation?.individualId || crd;
					} catch {}
				}
				if (!crd) continue;
				const c =
					...
					null;
				if (!c) continue;
				const normalized = normalizeIndividualPayload(c);
				const pid = personId(String(crd));
				const existing = mergedNodeMap.get(pid) || {};
				mergedNodeMap.set(pid, { ...existing, ...normalized, _trustedCurrentRelationshipData: true });
			}
		}
	} catch (e) {
		// non-fatal
