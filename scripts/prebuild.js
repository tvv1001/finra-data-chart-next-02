...
function runPrimedCacheBuild() {
	execSync('node .local/scripts/build_primed_cache_bundle.js', {
		stdio: 'inherit',
		env: {
			...process.env,
...
if (url && token) {
	...
	execSync('node .local/scripts/build_workers.js', {
		stdio: 'inherit',
	});
	...
	if (canRebuildSearchIndexes()) {
		execSync('node .local/scripts/build_search_indexes.js', {
			stdio: 'inherit',
		});
		runPrimedCacheBuild();
	} else {
